...
def new_connection_prompt(stdscr):
    ...
    while True:
        ...
        key = stdscr.getch()
        # shortcuts
        if key == ord('s'): curr_option = 0
        elif key == ord('j'): curr_option = 1
        elif key == ord('q'): curr_option = 2
        # up and down arrows
        elif key == curses.KEY_UP and curr_option > 0: curr_option -= 1
        elif key == curses.KEY_DOWN and curr_option < len(options) - 1: curr_option += 1
        # enter. break the true loop to call the funcion
        elif key == curses.KEY_ENTER or key in [10, 13]: break
